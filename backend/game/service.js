...
room.players[idx].where = null;
        ...
        async handlePlayCard({ code, playerId, cardIndex }) {
            ...
            if (
            game.blockedColorPlayer === playerId &&
            game.blockedColorTurns > 0 &&
            card.color === game.blockedColor
            ) {
            return false; // No puede jugar cartas de ese color
            }
...
setWhere(socket, code, where) {
 ...
 function refillDrawPileIfNeeded(game, threshold = 4) {
    ...
    // ── LUMOS REVEAL ──────────────────────────────────────────────────────
    if (card.frame === 'lumos.svg') {
        // Busca el oponente
        const playerIds = Object.keys(game.hands);
        const opponentId = playerIds.find(pid => pid !== playerId);
        if (opponentId) {
            // Selecciona 2 cartas aleatorias de la mano del oponente
            const oppHand = game.hands[opponentId];
            const shown = oppHand.slice(0, 2); // puedes usar random si prefieres
            // Envía solo al jugador que tiró la carta la info de las cartas
            io.to(game.players.find(p => p.playerId === playerId).socketId)
              .emit('lumos-reveal', { cards: shown });
        }
        // Continúa el flujo normal (no termina el turno aquí)
    }

    // ── PERDER APUESTA ────────────────────────────────────────────────
    if (card.frame === 'perder_apuesta.svg') {
    const playerIds = Object.keys(game.hands);
    if (playerIds.length === 2) {
        // Intercambia las manos
        const temp = game.hands[playerIds[0]];
        game.hands[playerIds[0]] = game.hands[playerIds[1]];
        game.hands[playerIds[1]] = temp;
        // Notifica a ambos jugadores que sus manos han cambiado
        playerIds.forEach(pid => {
            io.to(game.players.find(p => p.playerId === pid).socketId)
              .emit('update-hand', { hand: game.hands[pid] });
        });
    }
    
    //── 1PA1 SIN CAMISETA ────────────────────────────────────────────────
    if (card.frame === '1pa1_sin_camiseta.svg') {
        // Marca que estamos en modo duelo
        game.duelActive = true;
        // Notifica a ambos jugadores que deben tirar su carta más grande
        Object.values(game.players).forEach(player => {
            io.to(player.socketId).emit('duel-start');
        });
        return; // Pausa el flujo normal hasta que termine el duelo
    }

    //── MULTA ────────────────────────────────────────────────
    if (card.frame === 'multa.svg') {
    // Identifica el oponente
    const playerIds = Object.keys(game.hands);
    const opponentId = playerIds.find(pid => pid !== playerId);
    if (opponentId) {
        // Busca el último color jugado por el oponente
        const lastColor = game.lastColorPlayed?.[opponentId];
        if (lastColor) {
            // Marca el efecto de multa para el oponente durante 2 turnos
            game.multaEffect = {
                playerId: opponentId,
                color: lastColor,
                turns: 2
            };
            io.to(game.players.find(p => p.playerId === opponentId).socketId)
              .emit('multa-applied', { color: lastColor, turns: 2 });
        }
    }
    }
}
}
//── 1PA1 SIN CAMISETA MODAL ────────────────────────────────────────────────
io.on('connection', (socket) => {
  // ...existing code...
  socket.on('duel-card', ({ card }) => {
    const playerId = getPlayerIdBySocket(socket.id);
    if (!game.duelCards) game.duelCards = {};
    game.duelCards[playerId] = card;
    // Cuando ambos han enviado su carta
    if (Object.keys(game.duelCards).length === 2) {
      const [p1, p2] = Object.keys(game.duelCards);
      const c1 = game.duelCards[p1];
      const c2 = game.duelCards[p2];
      // El que tenga la carta más pequeña roba 2 cartas
      const v1 = Number(c1.value);
      const v2 = Number(c2.value);
      let loserId = null;
      if (v1 < v2) loserId = p1;
      else if (v2 < v1) loserId = p2;
      if (loserId) {
        for (let i = 0; i < 2; i++) {
          game.hands[loserId].push(game.deck.pop());
        }
        io.to(game.players.find(p => p.playerId === loserId).socketId)
          .emit('update-hand', { hand: game.hands[loserId] });
        io.to(game.players.find(p => p.playerId === loserId).socketId)
          .emit('duel-lost');
      }
      // Limpia estado de duelo
      game.duelActive = false;
      game.duelCards = {};
      // Continúa el turno normal
      // ...puedes llamar a la función de siguiente turno aquí...
    }
  });
  // ...existing code...
});
