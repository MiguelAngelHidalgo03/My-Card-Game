...
export function createDeck(enableSpecialCards) {
  ...
  if (enableSpecialCards) {
    ...
    // Cartas especiales adicionales
    deck.push({ frame: 'lumos.svg' });
    deck.push({ frame: 'perder_apuesta.svg' });
    deck.push({ frame: '1pa1_sin_camiseta.svg' });
    deck.push({ frame: 'multa.svg' });
    deck.push({ frame: 'ten_huevos_robar.svg' });
    deck.push({ frame: 'misteriosa.svg' });
  }
  return deck;
}
